...
| Read-only switch | Proven | Defaults per context to read-only. Packaged E2E verifies disabled controls and direct renderer attempts to bypass UI: Electron main rejects both Pod exec and resource mutate and the Kubernetes fixture receives neither request. |
...
| Terminal | Proven for approved v0.1 one-shot scope | Renderer exposes a bounded argv-only Pod command; Electron main enforces read-only; Go validates argv and caps stdout/stderr; the Kubernetes client uses SPDY exec with no stdin or TTY. Persistent interactive PTY/follow mode is explicitly outside v0.1. |
